...
@Composable
fun MainScreen(csvFile: File, navController: NavHostController) {
    ...
    val filteredData = csvData.drop(1).filter { row ->
        val (name, typeInvite, checkIn) = row
        val matchesSearch = name.contains(searchQuery, ignoreCase = true) || typeInvite.contains(searchQuery, ignoreCase = true)
    ...
    val checkedInCount = csvData.drop(1).count { it[2].isNotBlank() } // Check-in feito
    val notCheckedInCount = csvData.drop(1).count { it[2].isBlank() } // Check-in pendente

    // Layout da tela
    Scaffold (
        topBar = {
            TopAppBar(
                title = { Text(text = "Lista de Convidados", color = MaterialTheme.colorScheme.onBackground) },
                actions = {
                    IconButton(
                        onClick = {
        ...
        Column(
            ...
            LazyColumn {
                items(filteredDataWithoutHeader) { row  ->
                    val (name, ticketType, checkIn) = row
                    ListItem(
                        headlineContent = { Text(name) },
                        supportingContent = { Text(ticketType)},
                            ...
                            if (checkIn.isBlank()) {
                                if (isCheckingIn) {
                                    ...
                                    onClick = {
                                        ...
                                        if (index != -1) {
                                            updatedData[index][2] = currentTime
